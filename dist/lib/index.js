...
class ServerlessIamPerFunctionPlugin {
    ...
    getFunctionRoleName(functionName) {
        ...
        if (!lodash_1.default.isArray(fnJoin) ||
            fnJoin.length !== 2 ||
            !lodash_1.default.isArray(fnJoin[1]) ||
            fnJoin[1].length < 2) {
            this.throwError("Global Role Name is not in expected format. Got name: " +
    ...
    updateFunctionResourceRole(functionName, roleName, globalRoleName) {
        ...
        if (lodash_1.default.isEmpty(functionResource) ||
            ...
            this.throwError("Function Resource is not in expected format. For function name: " +
                functionName);
        }
    ...
    getStreamStatements(functionObject) {
        const res = [];
        if (lodash_1.default.isEmpty(functionObject.events)) {
            // no events
            return res;
        }
    ...
    createRoleForFunction(functionName, functionToRoleMap) {
        ...
        // set vpc if needed
        if (!lodash_1.default.isEmpty(functionObject.vpc) ||
            !lodash_1.default.isEmpty(this.serverless.service.provider.vpc)) {
            functionIamRole.Properties.ManagedPolicyArns = [
                {
                    "Fn::Join": [
                        "",
        ...
        if (isInherit && !lodash_1.default.isEmpty(providerIamRoleStatements)) {
    ...
    createRolesPerFunction() {
        ...
        // delete unused role where has been set for individual function
        const globalRoleName = this.serverless.providers.aws.naming.getRoleLogicalId();
        delete this.serverless.service.provider.compiledCloudFormationTemplate
            .Resources[globalRoleName];
    }
}
